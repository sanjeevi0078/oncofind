...
\end{equation}
This correction dampens the false discovery rate (FDR) and prevents prognostic p-hacking.
...
\caption{Kaplan-Meier overall survival analysis generated by \texttt{oncofind} for the high- vs. low-expression cohorts. The plot overlays the survival percentiles, confidence intervals, and the Bonferroni-corrected log-rank test statistical significance ($p_{adjusted}$).}
\label{fig:survival}
\end{figure}
...
\caption{Pan-cancer expression level comparison of \textit{TP53} across BRCA, LUAD, and COAD cohorts showing batch-standardized expression profiles across tumor and normal samples.}
\label{fig:pancancer}
\end{figure}
